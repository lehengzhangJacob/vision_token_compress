...
Following the qualitative figures in the three papers, we reproduce their token-selection visualizations on new videos (a cooking clip, a cycling race, a Video-MME astronomy video, and a long MLVU film clip; none are the papers' original examples). The selection decisions are captured from the same instrumented code paths used in the quantitative reproduction. One example per method is shown below; the full set is under \texttt{visualizations/out/}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{visualizations/out/aot_anchors_cooking_muffins.png}
\caption{AOT local/global token anchors on a cooking video (LLaVA-OV-7B, VTN$=126/729$ per frame). Top: sampled frames; bottom: \textcolor[RGB]{30,150,80}{green} = Local anchors (shallow-layer window CLS attention), \textcolor[RGB]{220,120,0}{orange} = Global anchors (deep-layer CLS attention); unselected patches are dimmed and later merged into the anchors via optimal transport. Anchors concentrate on the girl's face, hands, and the chocolate she is dipping.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{visualizations/out/prunevid_cycling_race.png}
\caption{PruneVid stages on a cycling-race video (PLLaVA-7B, 16 frames, $12\times12$ grid). Row 2: static (\textcolor[RGB]{70,130,220}{blue}) vs.\ dynamic (\textcolor[RGB]{220,120,0}{orange}) patches ($\tau=0.8$) --- the moving peloton is dynamic while road and background are static and merged temporally. Row 3: layer-10 question-to-visual attention broadcast back to patches. Row 4: tokens kept by the LLM-stage top-$\alpha$ selection ($\alpha=0.4$), which tracks the cyclists.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{visualizations/out/vidcom2_uniqueness_videomme_08km9Yqbt.png}
\caption{VidCom$^2$ frame uniqueness and token allocation on a Video-MME astronomy video (LLaVA-OV-7B, $R=25\%$). Taller/brighter bars mark more unique frames, which receive a larger per-frame retention ratio $r_t$ (labels above bars); visually redundant frames are compressed harder. Bottom: the retained $14\times14$ patches per frame.}
\end{figure}
