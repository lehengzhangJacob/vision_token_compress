...
\rowcolor{green!8}
LLaVA-Video-7B & VidCom$^2$, $R=25\%$ (\repro) & 56.88 & 56.62 & \na & 61.85 & 72.89 & 61.67 & 51.00 & \na \\
...
\rowcolor{green!8}
LLaVA-Video-7B & VidCom$^2$, $R=15\%$ (\repro) & 53.33 & 52.28 & \na & 58.22 & 68.00 & 56.89 & 49.78 & \na \\
\bottomrule
\end{tabular}}
\caption{VidCom$^2$ main experiment table with local LLaVA-OV and LLaVA-Video reproduction rows inserted below the corresponding paper rows. MLVU (both models) is still pending (large dataset download); Average(\%) is left blank until the MLVU column is available.}
...
\rowcolor{green!8}
AOT (\repro) & 8.7 & 21.3\% & 25\%$^\S$ & 57.75 & 61.18$^\dagger$ & 54.75 & 54.89 & 57.14 & 97.8 \\
...
\rowcolor{green!8}
AOT (\repro) & 7.0 & 17.2\% & 20\%$^\S$ & 57.73 & 61.22$^\dagger$ & 54.08 & 54.78 & 56.95 & 97.5 \\
...
\rowcolor{green!8}
AOT (\repro) & 3.4 & 8.3\% & 15\%$^\S$ & 57.33 & 60.98$^\dagger$ & 52.80 & 54.22 & 56.33 & 96.5 \\
...
\rowcolor{green!8}
AOT (\repro) & 5.2 & 12.7\% & 10\%$^\S$ & 56.70 & 60.45$^\dagger$ & 52.65 & 53.78 & 55.90 & 95.7 \\
\bottomrule
\end{tabular}}
\caption{AOT main experiment table on LLaVA-OneVision with local reproduction rows inserted below the corresponding paper AOT rows. $^\dagger$EgoSchema full-test labels are not public; accuracy is obtained by submitting the local predictions to the official validation server (the same predictions also agree with the authors' committed submissions on 94--99\% of items). $^\S$LongVideoBench and VideoMME reproduction values come from a completed VTN-calibrated sweep in which the AOT visual-token budget was tuned (VTN $=146/208/267/322$) so the \emph{measured} token retention matches the labeled $10/15/20/25\%$; a token-retention probe had shown the earlier VTN settings retained only $\sim$half the labeled ratio (e.g.\ the old $25\%$ LongVideoBench $52.80$ was actually $\sim$13\% retained). Calibration raises LongVideoBench (e.g.\ $52.80\rightarrow54.75$ at $25\%$) but barely moves VideoMME, leaving a residual $\sim$1.5--2.6 point gap to the paper on the long-video benchmarks even at matched retention; MVBench and EgoSchema use the original runs, which already matched the paper closely. Averages are computed over the four benchmarks, with Avg.\ \% relative to the full-model baseline.}
