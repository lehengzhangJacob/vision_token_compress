...
from models.tarsier import TarsierForConditionalGeneration, LlavaConfig, Processor, LlavaProcessor, TarsierFLowForConditionalGeneration
...
def setup_model(
    ...
    # processor = PllavaProcessor.from_pretrained('MODELS/llava-1.6', 
    #                                            padding_side='right', 
    ...
    #     max_n_frames=config.model.num_frames,
    # )
    ...
    model = TarsierFLowForConditionalGeneration.from_pretrained(
        config.model.repo_id,
        config=model_config,
        torch_dtype=torch_dtype
    ...
    flow_checkpoint = '~/.cache/mim/pwcnet_ft_4x1_300k_sintel_final_384x768.pth'
    ...
    if not config.model.freeze_lm:
        for n, p in model.language_model.named_parameters():
            if 'all' in config.model.lm_unfreeze_modules:
    ...
    if not config.model.freeze_projector:
        for n, p in model.multi_modal_projector.named_parameters():
            if 'all' in config.model.projector_unfreeze_modules:
                p.requires_grad = True
                continue
            for key in config.model.projector_unfreeze_modules:
                if key in n:
                    p.requires_grad = True
                    break
                else:
