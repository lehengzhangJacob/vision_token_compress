...
def setup_model(
    ...
    model = PllavaFlowForConditionalGeneration.from_pretrained(config.model.repo_id, config=model_config, torch_dtype=torch_dtype)
    ...
    try:
        ...
        logger.info("Successfully loading optical flow model")
    except:
        logger.info("Loading optical flow model failed")
