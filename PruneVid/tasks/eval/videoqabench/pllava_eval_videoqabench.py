...
def load_model_and_dataset(rank, world_size, pretrained_model_name_or_path, num_frames, use_lora, lora_alpha, weight_dir, test_ratio, test_datasets, pooling_shape, selected_layer=10, alpha=0.1, softmax=1.0, head=0):
    # remind that, once the model goes larger (30B+) may cause the memory to be heavily used up. Even Tearing Nodes.
    model, processor = load_pllava(pretrained_model_name_or_path, num_frames=num_frames, use_lora=use_lora, lora_alpha=lora_alpha,\
         weight_dir=weight_dir, pooling_shape=pooling_shape, selected_layer=selected_layer, alpha=alpha, softmax=softmax, head=head)
