...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="fullset",
        choices=["subset", "fullset", "both"],
        help="subset=500 labeled questions; fullset=5031 for server upload",
    )
    args = parser.parse_args()
    return args

def load_model_and_dataset(rank, world_size, pretrained_model_name_or_path, num_frames, use_lora, lora_alpha, weight_dir, pooling_shape=(16,12,12), selected_layer=10, alpha=0.1, softmax=1.0, head=0, tau=1.0, cluster_ratio=1.0, temporal_segment_ratio=1.0, eval_split="subset"):
    ...
    dataset = EgoSchemaDataset(num_segments=num_frames, eval_split=eval_split)
