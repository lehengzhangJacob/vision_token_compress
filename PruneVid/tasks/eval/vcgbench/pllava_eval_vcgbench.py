...
def load_model_and_dataset(rank, world_size, pretrained_model_name_or_path, num_frames, use_lora, lora_alpha, weight_dir, test_ratio, pooling_shape=(16,12,12), selected_layer=10, alpha=0., head=0, softmax=1., tau=1.0, cluster_ratio=1.0, temporal_segment_ratio=1.0):
    # remind that, once the model goes larger (30B+) may cause the memory to be heavily used up. Even Tearing Nodes.,
    model, processor = load_pllava(pretrained_model_name_or_path, num_frames=num_frames, use_lora=use_lora, weight_dir=weight_dir, lora_alpha=lora_alpha, pooling_shape=pooling_shape, selected_layer=selected_layer, \
        alpha=alpha, head=head, softmax=softmax, tau=tau, cluster_ratio=cluster_ratio, temporal_segment_ratio=temporal_segment_ratio)
...
def run(rank, args, world_size,start_rank=0):
    ...
    if rank == 0:
        ...
        tbar = tqdm(total=len(dataset.data_list))
...
def main():
    ...
    result_list, flag = load_results(save_path)
    start_rank=0
