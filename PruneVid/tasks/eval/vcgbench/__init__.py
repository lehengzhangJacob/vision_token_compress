...
api_key_list = [_api_key] if _api_key else []

client = OpenAI(api_key=_api_key, base_url="https://api.openai.com/v1/") if _api_key else None
...
def save_results(result_list, save_path, model="gpt-3.5-turbo-0125"):
    ...
    if client is None:
        print("OPENAI_API_KEY not set; saved inference_results.json only (skip GPT scoring)")
        return
