...
class Visualizer:
    ...
    def draw_binary_mask_with_number(
        self, binary_mask, color=None, *, edge_color=None, text=None, label_mode='1', alpha=0.1, anno_mode=['Mask'], area_threshold=10, font_size=18
    ):
        """
        Args:
            binary_mask (ndarray): numpy array of shape (H, W), where H is the image height and
        ...
        if text is not None and has_valid_segment:
            ...
            self._draw_number_in_mask(binary_mask, text, lighter_color, label_mode, font_size=font_size)
        return self.output
    ...
    def _draw_number_in_mask(self, binary_mask, text, color, label_mode='1',font_size=18):
        """
        ...
        self.draw_text(text, (coords_x[len(coords_x)//2] + 2, coords_y[len(coords_y)//2] - 6), color=color, font_size=font_size)
