...
class PllavaMultiModalProjector(nn.Module):
    ...
    def __init__(self, config: PllavaConfig):
        ...
        # self.pooling_2d = nn.AdaptiveAvgPool3d((1, *config.pooling_shape[-2:]))
        self.pooling_S = nn.AdaptiveAvgPool3d((self.slow_frames, 12, 24))
        self.pooling_F = nn.AdaptiveAvgPool3d((self.num_frames, 4, 4))
    ...
    def forward(self, image_features, media_type, batch_size=None, num_videos=None):
        ...
        # hidden_states = torch.cat([hidden_states, global_hidden_states], dim=0) # b l d
        ...
        slow_idx = range(0, self.num_frames-1, self.slow_frames)
        hidden_states_videos_slow = hidden_states_videos[:,:,slow_idx,:,:]
        hidden_states_videos_slow = self.pooling_S(hidden_states_videos_slow)

        hidden_states_videos_fast = self.pooling_F(hidden_states_videos)

        hidden_states_videos_fast = einops.rearrange(hidden_states_videos_fast, 'batch_size_num_videos embed_dims num_frames h w -> batch_size_num_videos (num_frames h w) embed_dims ')
        hidden_states_videos_slow = einops.rearrange(hidden_states_videos_slow, 'batch_size_num_videos embed_dims num_frames h w -> batch_size_num_videos (num_frames h w) embed_dims ')

        hidden_states = torch.cat([hidden_states_videos_fast, hidden_states_videos_slow], dim=1)
...
class PllavaSFForConditionalGeneration(PllavaPreTrainedModel):
    def __init__(self, config: PllavaConfig):
