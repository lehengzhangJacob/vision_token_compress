...
class STLLMModel(Blip2Base):
    ...
    def apply_prunevid_merge(self, img_embeds):
        ...
        merged, static_sizes, dynamic_sizes, window_sizes = merger.merge_frames_dynamic(
            img_embeds.reshape(batch_size, frame_count * tokens_per_frame, hidden_size)
        )
        ...
        return merged.view(batch_size, 1, merged.shape[1], hidden_size)
    ...
    def forward(self, samples):
        ...
        if prunevid_enabled and not use_image and self.video_input == 'all':
            img_embeds = self.apply_prunevid_merge(img_embeds)
            atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(image.device)
        elif not use_image and self.video_input == 'all':
        ...
        if not use_image and self.use_mask and not prunevid_enabled:
            self.img_len = L
